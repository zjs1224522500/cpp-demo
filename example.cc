...
int main()
{
        ...
        Num n(0xFFFFFFF);
        Num* q = new Num(0xFFFFFFF);

        // 正常执行的任务
        pool->add_task(hello);
        // 延时任务
        pool->delay_task(1000, []()
                         { delay(0); });

        // future任务，等待返回值
        auto return_future = std::move(pool->add_future_task(std::mem_fn(&Num::summary), &n));
        auto fn = std::mem_fn(&Num::sum_to_n);
        // auto return_future_2 = std::move(pool->add_future_task(fn, &n, 10));
        auto return_future_2 = std::move(pool->add_future_task(fn, q, 100));
        pool->add_future_task(fn, q, 100);
        ...
        uint64_t sum2_val = return_future_2.get();

        g_log_mutex.lock();
        std::cout << LogPrefix() << "Summary result: " << sum_val << std::endl;
        std::cout << LogPrefix() << "Summary2 result: " << sum2_val << std::endl;
        g_log_mutex.unlock();
