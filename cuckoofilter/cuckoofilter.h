...
class CuckooFilter {
 ...
 public:
  explicit CuckooFilter(const size_t max_num_keys) : num_items_(0), victim_(), hasher_() {
    // fprintf(stderr, "CuckooFilter::CuckooFilter()\n");
    size_t assoc = 4;
  ...
  void Clear() {
    // fprintf(stderr, "CuckooFilter::Clear()\n");
    num_items_ = 0;
    victim_.used = false;
    table_->Clear();
  }
...
Status CuckooFilter<ItemType, bits_per_item, TableType, HashFamily>::Add(
    const ItemType &item) {
  // fprintf(stderr, "CuckooFilter::Add()\n");
  size_t i;
  uint32_t tag;

  if (victim_.used) {
    fprintf(stderr, "CuckooFilter::Add() victim.used\n");
...
Status CuckooFilter<ItemType, bits_per_item, TableType, HashFamily>::AddImpl(
  ...
  fprintf(stderr, "CuckooFilter::Add() NotEnoughSpace and init victim\n");
  victim_.index = curindex; // 逐出多次之后还是有个数据没存下来，则将其存入 victim_
  fprintf(stderr, "CuckooFilter::Add() failed with size %zu and bytes %zu\n", num_items_, SizeInBytes());
